...
export default function CheerMeUpLifeMain() {
  ...
  useEffect(() => {
    ...
    getApprovedReviews()
      ...
      .catch((error) => console.error("승인 리뷰를 불러오지 못했습니다.", error));
    return () => {
      active = false;
    };
  }, []);
  ...
  return (
    <main className="w-full min-h-screen bg-gradient-to-b from-yellow-50 to-pink-100 font-sans overflow-x-hidden">

    {/* 1. Hero Section */}
    <section
      className="
        w-full
        pt-24 pb-12 md:pt-28
    ...
    <section className="max-w-6xl mx-auto py-16 px-6 grid md:grid-cols-2 gap-10 items-center">
      ...
      <div>
        ...
        <div><p className="text-lg text-gray-700 mb-4">누구나 처음! 동작부터 음악까지 <b>하루만에 공연 가능!</b></p>
        <p className="text-lg text-gray-700 mb-4">초등학생, 중고생, 대학생, 직장인, 시니어 / 남녀노소 입문자부터 경력자까지 모두 커버 가능</p></div>
    ...
    {/* 5. 상품(가격/플랜) 카드 */}
    <section id="products" className="w-full py-16 bg-gradient-to-r from-yellow-50 to-pink-100">
    ...
    <SharePrompt
      variant="home"
      title="같이 응원하면 시작이 더 즐거워집니다"
      description="치어리딩을 함께 배우고 싶은 친구나 공연을 준비하는 팀원에게 취미로운응원생활을 소개해보세요."
      buttonLabel="친구에게 소개하기"
      shareTitle="취미로운응원생활 | 치어리딩 레슨·공연·온라인 클래스"
      shareText="치어리딩을 처음 배워도 즐겁게 시작할 수 있는 취미로운응원생활을 소개해요."
      sharePath="/"
      location="home_before_final_cta"
    />
    ...
    <section className="w-full flex flex-col items-center justify-center py-12 bg-gradient-to-br from-yellow-200 to-pink-100">
      ...
      <button
        ...
        // }}
      >
        <span className="font-giantsInline text-3xl text-white-600">👉🏼 취미로운 응원레슨, 지금 바로 예약하기</span>
      </button>
    </section>
